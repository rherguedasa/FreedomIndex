...
if index == 'INTRODUCCIÓN':
    ...
    with tab_plots:
        st.header('DataSet')
        st.markdown('**DataSet extraido de la web Heritage, tras el preprocesamiento**')
...
#----------------------------------------------------------------------------------------------------------------TERCERA PESTAÑA: ANÁLISIS CORRELACIONAL------------------------------------------------------------------------------------------------------------------
    ...
    # hacemos un plot relacionando Financial Freedom con Investmen Freedom por países
    tab_plots = cor[4]
    with tab_plots:
        st.header('Libertad financiera: relación con la libertad de inversión por países')
        with st.expander("**¿Qué países tienen mayor libertad financiera?**"):
            st.write("""**Los países mejor rankeados tienen una mayor libertad finenciaria y facilidad para la inversión**""")
        fig = go.Figure()
...
#----------------------------------------------------------------------------------------------------------------CUARTA PESTAÑA: MODELO PREDICTIVO------------------------------------------------------------------------------------------------------------------
    ...
    country = st.text_input('Escribe el nombre de del país')

    if country in countries:
        # Buscar los datos del país seleccionado
        ...
        st.write("**For Linear Regression the Freedom Index Score prediction for {} is {} and the actual value in the Heritage Economic index score is {}**".format(country, round(predictionlin[0], 2), actual_value))
        st.write("**For Ridge Regression Freedom Index Score prediction for {} is {} and the actual value in the Heritage Economic index score is {}**".format(country, round(predictionrid[0], 2), actual_value))
        st.write("**For Lasso Regression the Freedom Index Score prediction for {} is {} and the actual value in the Heritage Economic index score is {}**".format(country, round(predictionlas[0], 2), actual_value))
